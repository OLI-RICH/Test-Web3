  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      <header className="border-b border-border bg-hero text-hero-foreground">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-5 md:px-10">
          <Link to="/" className="flex items-center gap-3 font-bold">
            <span className="grid size-9 place-items-center rounded-lg bg-primary text-primary-foreground"><CalendarDays className="size-5" /></span>
            Event<span className="text-primary">Sync</span>
          </Link>
          <Button variant="nav" size="sm" asChild><Link to="/">Événements</Link></Button>
        </div>
      </header>
      ...
      <section className="mx-auto max-w-5xl px-6 py-12 md:px-10">
        ...
        <h1 className="mt-2 font-display text-4xl font-bold tracking-normal">Votre sélection d’événements</h1>
        ...
        <div className="mt-8 grid gap-4">
          {items.length === 0 ? (
            <div className="rounded-lg border border-border bg-card p-6 text-card-foreground shadow-card">
              ...
              <Button className="mt-5" variant="pill" asChild><Link to="/">Découvrir les événements</Link></Button>
            </div>
          ) : items.map((item) => (
            <article key={item.id} className="rounded-lg border border-border bg-card p-5 text-card-foreground shadow-card">
              <div className="flex flex-col justify-between gap-4 md:flex-row md:items-start">
                <div>
                  ...
                  <p className="mt-2 flex items-center gap-2 text-sm text-muted-foreground"><MapPin className="size-4 text-primary" /> {item.location}</p>
                  <p className="mt-4 max-w-3xl text-sm leading-6 text-muted-foreground">{item.description}</p>
                </div>
                <Button variant="secondary" size="sm" onClick={() => removeItem(item.id)}><Trash2 className="size-4" /> Retirer</Button>
              </div>
            </article>
          ))}
        </div>
