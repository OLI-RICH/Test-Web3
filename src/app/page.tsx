...
import { useEffect, useState } from "react";
import Link from "next/link";
import { MapPin, ArrowRight, Search, LayoutGrid, Clock } from "lucide-react";
...
export default function Home() {
  ...
  // Gestion du scroll automatique si l'URL contient #events-section
  useEffect(() => {
    ...
    <div className="min-h-screen bg-white">
      ...
      <section className="relative min-h-[80vh] flex items-center overflow-hidden px-6 pt-40 pb-20">
        ...
        <div className="relative z-10 max-w-7xl mx-auto w-full">
          ...
          <h1 className="hero-title mb-10 text-white text-6xl md:text-8xl font-black leading-none tracking-tighter">
            Vivez vos <br />
            <span className="text-[#2ecc71] italic">événements</span> <br />
            en temps réel
          </h1>

          <div className="search-container flex items-center bg-white/10 backdrop-blur-md rounded-2xl border border-white/10 max-w-2xl overflow-hidden shadow-2xl">
            <Search className="w-5 h-5 text-white/30 ml-6" />
            <input 
              type="text" 
              placeholder="Rechercher un événement..." 
              className="flex-1 bg-transparent border-none px-4 py-6 text-sm text-white placeholder-white/40 outline-none"
              onChange={(e) => setSearchTerm(e.target.value)}
            />
            <button className="btn-primary bg-[#2ecc71] text-black font-black px-8 py-6 uppercase text-xs tracking-widest hover:bg-green-400 transition-colors">
              Rechercher
            </button>
          </div>
        </div>
      ...
      <section id="events-section" className="py-24 scroll-mt-24">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-16 gap-6">
            <div>
              <span className="text-[#2ecc71] font-black text-[10px] uppercase tracking-[0.3em] block mb-2">Catalogue</span>
              <h2 className="text-4xl md:text-6xl font-black tracking-tighter text-black">Tous les événements</h2>
              <p className="text-gray-400 text-sm mt-2">{filteredEvents.length} événements disponibles</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
            {filteredEvents.map((event) => (
              <div key={event.id} className="group bg-white rounded-[32px] overflow-hidden border border-gray-100 shadow-sm hover:shadow-2xl transition-all duration-500 flex flex-col h-full">
                <div className="relative h-72 overflow-hidden">
                  <div className="absolute top-6 left-6 z-10 bg-black/30 backdrop-blur-md text-white text-[10px] font-black px-4 py-2 rounded-full border border-white/20 uppercase tracking-widest">
                    15 JUIN 2026
                  </div>
                  <img 
                    src={event.imageUrl || "https://images.unsplash.com/photo-1540575861501-7cf05a4b125a?q=80&w=2070"} 
                ...
                <div className="p-10 flex flex-col flex-1">
                  <div className="flex items-center gap-2 text-gray-400 mb-6 font-bold text-xs uppercase tracking-widest">
                    <MapPin className="w-4 h-4 text-[#2ecc71]" />
                    <span>{event.location}</span>
                  </div>
                  ...
                  <div className="mt-auto pt-8 border-t border-gray-50 flex items-center justify-between">
                    ...
                    <Link 
                      href={`/events/${event.id}`} 
                      className="text-[#2ecc71] font-black flex items-center gap-2 group-hover:gap-4 transition-all uppercase text-[10px] tracking-widest"
                    >
                      Programme <ArrowRight className="w-4 h-4" />
                    </Link>
                  </div>
